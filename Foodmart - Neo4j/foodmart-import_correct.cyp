CREATE CONSTRAINT FOR (r:Region) REQUIRE r.id IS UNIQUE;
CREATE CONSTRAINT FOR (r:Region) REQUIRE r.name IS UNIQUE;
CREATE CONSTRAINT FOR (c:Customer) REQUIRE c.id IS UNIQUE;
CREATE CONSTRAINT FOR (c:Customer) REQUIRE c.account_num IS UNIQUE;
CREATE CONSTRAINT FOR (pf:ProductFamily) REQUIRE pf.name IS UNIQUE;
CREATE CONSTRAINT FOR (d:Department) REQUIRE d.name IS UNIQUE;
CREATE CONSTRAINT FOR (pc:ProductCategory) REQUIRE pc.name IS UNIQUE;
CREATE CONSTRAINT FOR (ps:ProductSubCategory) REQUIRE ps.id IS UNIQUE;
CREATE CONSTRAINT FOR (b:Brand) REQUIRE b.name IS UNIQUE;
CREATE CONSTRAINT FOR (p:Product) REQUIRE p.id IS UNIQUE;
CREATE CONSTRAINT FOR (st:StoreType) REQUIRE st.name IS UNIQUE;
CREATE CONSTRAINT FOR (s:Store) REQUIRE s.id IS UNIQUE;
CREATE CONSTRAINT FOR (y:Year) REQUIRE y.year IS UNIQUE;
CREATE CONSTRAINT FOR (m:Month) REQUIRE m.id IS UNIQUE;
CREATE CONSTRAINT FOR (d:Date) REQUIRE d.id IS UNIQUE;
CREATE CONSTRAINT FOR (d:Date) REQUIRE d.date IS UNIQUE;
CREATE CONSTRAINT FOR (d:Date) REQUIRE d.day IS UNIQUE;
CREATE CONSTRAINT FOR (p:Promotion) REQUIRE p.id IS UNIQUE;
CREATE CONSTRAINT FOR (s:Sale) REQUIRE s.id IS UNIQUE;
...
, s.coffee_bar = toInteger(line.coffee_bar) > 0
, s.video_store = toInteger(line.video_store) > 0
, s.salad_bar = toInteger(line.salad_bar) > 0
, s.prepared_food = toInteger(line.prepared_food) > 0
, s.florist = toInteger(line.florist) > 0
MERGE (s)-[:OF_TYPE]->(st)
MERGE (s)-[:IN_CITY]->(c)
MERGE (c)-[:IN_REGION]->(r)
;
...
MERGE (y:Year {year: toInteger(line.the_year)})
MERGE (m:Month {id: line.the_year + "-" + line.the_month})
ON CREATE
SET m.month = toInteger(line.the_month)
MERGE (m)<-[:HAS_YEAR]-(y)
MERGE (d:Date {id: line.time_id})
ON CREATE
SET d.date = line.the_date
, d.day = toInteger(line.day_since_epoch)
, d.day_of_month = toInteger(line.day_of_month)
...
ON CREATE
SET i.price = toInteger(replace(line.store_sales, ".", ""))
, i.cost_price = toInteger(replace(line.store_cost, ".", ""))
, i.quantity = toInteger(replace(line.unit_sales, ".", ""))
WITH s, line
